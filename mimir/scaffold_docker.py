    ...
    if skills_root.is_dir():
        for skill_dir in sorted(skills_root.iterdir()):
            ...
            try:
                ...
                continue
            for p in spec.get("pollers", []):
                for var in p.get("pass_env", []):
...
_DOCKERFILE_BASE = """\
# Generated by `mimir scaffold-docker`. Re-run to refresh skill-fragment
# block + base template; operator edits OUTSIDE the sentinel-marked
# blocks are preserved across regenerations (we only overwrite the
# sentinel-bounded sections).
#
...
def render_dockerfile(fragments: list[Fragment]) -> str:
    """Stitch fragments into the base template."""
    if not fragments:
        ...
        body = "\n\n".join(parts)
    return _DOCKERFILE_BASE.replace("{FRAGMENTS}", body)


_COMPOSE_YML_TEMPLATE = """\
# Generated by `mimir scaffold-docker`. Re-run to refresh; operator
# edits to this file get overwritten on each run.
#
# Usage:
...
_START_SH = """\
#!/usr/bin/env bash
# Generated by `mimir scaffold-docker`. Re-run to refresh.
#
...
# ─── deps sync ─────────────────────────────────────────────────────
echo "[start.sh] uv sync (extras: discord, claude-code)"
uv sync --extra discord --extra claude-code
...
def scaffold(
    ...
    *,
    ...
    cy_path = home / "compose.yml"
    cy_path.write_text(cy_text)
    result.files_written.append("compose.yml")

    # start.sh — full regen.
    ss_path = home / "start.sh"
    ss_path.write_text(render_start_sh())
    ss_path.chmod(0o755)
    result.files_written.append("start.sh")
