...
def package_entrypoint(tmp_path: Path) -> Path:
    ...
    (feature / "package.json").write_text(
        json.dumps({"name": "feature-factory", "version": "0.9.2"}),
        encoding="utf-8",
    )
    (adapter / "package.json").write_text(
        json.dumps({"name": "opencode-feature-factory", "version": "0.9.2"}),
        encoding="utf-8",
    )
    return entrypoint
...
def structured_status_payload() -> dict[str, Any]:
    # 0.9.2 release shape, including Gate 3's commit binding, not a live probe.
