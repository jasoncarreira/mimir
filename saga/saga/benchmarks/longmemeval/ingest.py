...
def ingest_question(question: dict, batch_size: int = 256) -> dict:
    ...
    Returns stats.
    """
    from saga.core import store_atom, store_session_boundary, get_db
    ...
    boundary_stored = 0
    from saga.config import get_config as _gc
    ...
    # P35: triples are now extracted *during consolidation* (one LLM
    # call per cluster produces both the observation and the triples).
    # The standalone P7 batch_extract_and_store path is no longer in
    # the bench hot path; triples_extracted will be reported by the
    # consolidate phase instead via cresult["triples_persisted"].
