...
def test_render_dockerfile_gates_claude_code_cli_on_build_arg():
    ...
    for mode in ("workspace", "pypi"):
        ...
        assert "npm install -g @anthropic-ai/claude-code@2.1.195" in out, mode
        assert "@anthropic-ai/claude-code@2.1.195 @mermaid-js/mermaid-cli" not in out, mode
...
def test_render_dockerfile_installs_codex_when_enabled():
    ...
    for mode in ("workspace", "pypi"):
        ...
        assert "npm install -g @openai/codex@0.142.4" in out, mode
...
def test_scaffold_installs_codex_for_codex_plus_extra(tmp_path: Path):
    ...
    assert "npm install -g @openai/codex@0.142.4" in (home / "Dockerfile").read_text()
