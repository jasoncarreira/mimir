...
`web_search` results, fetched body content read from the fetch cache, typed
PR/repository reads, and untrusted poller trigger payloads. Reads under
agent-writable roots, shell/process/Hands/REPL output, failures and refusal text,
trusted content, and
model-supplied tool arguments never contribute. The server keeps at most 1000
