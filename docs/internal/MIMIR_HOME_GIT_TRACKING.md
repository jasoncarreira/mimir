...
apply_git_identity() {
  ...
  # noreply@mimir-agent.local with no GitHub-account association — commits will
  ...
  git -C /mimir-home config user.email "noreply@mimir-agent.local"
}
...
3. **Committer identity:** `user.name = "mimir"`, `user.email =
   "noreply@mimir-agent.local"`. The email has no associated GitHub account
