...
class TestKendallTau:
    def test_perfect_agreement(self):
        from saga.calibration import _kendall_tau
        ranking = ["a", "b", "c", "d", "e"]
...
class TestOverlapAtK:
    def test_identical_rankings(self):
        from saga.calibration import _overlap_at_k
        ranking = ["a", "b", "c", "d", "e"]
...
class TestRankAtomsByQuery:
    def test_ranks_by_similarity(self):
        from saga.calibration import _rank_atoms_by_query
...
class TestCalibrateEmpty:
    def test_calibrate_empty_db(self, monkeypatch):
        ...
        from saga.calibration import calibrate
        ...
        monkeypatch.setattr("saga.calibration._instantiate_provider", lambda n: FakeProvider())
        monkeypatch.setattr("saga.calibration.get_provider", lambda: FakeProvider())
...
class TestReEmbed:
    ...
    def test_actual_reembed(self, monkeypatch):
        from saga.core import get_db, run_migrations, store_atom, unpack_embedding
        from saga.calibration import re_embed
