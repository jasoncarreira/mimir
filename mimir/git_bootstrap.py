...
# Pre-OSS hardening (review item #8). The bash tool can land arbitrary
# stdout / stderr into ``turns.jsonl`` — and the git-tracking module
# auto-commits ``turns.jsonl``, so anything the bash subprocess emits
# becomes a public commit message worth of disclosure surface. The
# original ``_redact`` only handled GitHub PATs; that left every other
# common token shape (Anthropic, Slack, generic bearer/token=) leaking
# verbatim if the agent ran something like ``env | grep TOKEN``. The
# patterns below cover the credentials that mimir's own bridges and
# providers use today plus the conventional ``Authorization: Bearer``
# / ``token=`` / ``password=`` shapes. False positives are acceptable
# (they're additive: the redactor never refuses to log, just masks).
_TOKEN_PATTERNS: tuple[re.Pattern[str], ...] = (
    _PAT_REGEX,
    # Anthropic API keys. Prefix ``sk-ant-`` is stable across the API
    # and Claude Code provisioning paths. Allow the underscore /
    # hyphen alphabet observed in issued keys.
    ...
    # OpenAI-style secret keys (``sk-…`` and ``sk-proj-…``). The
    # ``sk-ant-`` case is already covered above; this matches the
    # OpenAI shapes without colliding.
    re.compile(r"sk-(?!ant-)[A-Za-z0-9_-]{20,}"),
    # ``Authorization: Bearer <token>`` headers in dumped HTTP traces.
    # Case-insensitive; captures the value through whitespace / quote
    # boundary.
    re.compile(r"(?i)(bearer\s+)([A-Za-z0-9._~+/=-]{8,})"),
    # ``token=…``, ``api_key=…``, ``password=…`` value-style fields
    # (URL query, env var dumps, JSON pretty-prints with bareword keys).
    # The value alphabet stops at common delimiters so the regex doesn't
    # eat the rest of the line.
    re.compile(r"(?i)(token=|api[_-]?key=|password=|passwd=|secret=)([^\s\"',&]+)"),
    # Discord bot tokens — JWT-shaped with ``MTk…`` / ``MzU…`` prefix
    # for many of them. Use the well-documented 24+.6+27 segment shape.
...
def _redact(text: str) -> str:
    ...
    Pre-OSS hardening (review item #8): the original ``_redact`` only
    handled GitHub PATs. Bash output containing ``ANTHROPIC_API_KEY``,
    ``SLACK_BOT_TOKEN``, ``DISCORD_TOKEN``, ``Authorization: Bearer
    …``, ``token=…`` etc. used to land verbatim in ``turns.jsonl``,
    which the git-tracking module auto-commits. The extended pattern
    list covers the conventional shapes for credentials mimir's own
    bridges and providers issue.

    Replacement is ``[REDACTED]`` so logs still indicate "something
    matched a token shape here" without exposing the value. For the
    ``bearer …`` and ``token=…`` patterns, the prefix is preserved
    so the surrounding context is still readable.
    """
    if not text:
        return text
    out = text
    for pat in _TOKEN_PATTERNS:
        # Patterns with capture groups preserve the prefix; the others
        # mask the whole match. We detect by group count.
        if pat.groups == 2:
