...
def _opencode_install_block(install_opencode: bool) -> str:
    ...
    return (
        ...
        f"# Pins: opencode-ai@{OPENCODE_VERSION}, feature-factory@0.7.3,\n"
        "# opencode-feature-factory@0.7.3,\n"
...
#
# Usage (secrets go in compose.env; build toggles go in .env or the shell):
#   edit compose.env
#   printf 'MIMIR_ENABLE_CLAUDE_CODE=1\n' > .env  # optional build toggle
...
services:
  {SERVICE_NAME}:
    ...
    env_file:
      - compose.env
    environment:
      # These use the same interpolation as build.args so build-time and
...
def _existing_scaffold_settings(
    home: Path,
) -> tuple[ScaffoldMode | None, int | None, str | None, bool | None]:
    """Recover settings from scaffolder-owned files for an omitted-flag rerun."""
    dockerfile = home / "Dockerfile"
    compose = home / "compose.yml"
    docker_text = dockerfile.read_text() if dockerfile.is_file() else ""
    compose_text = compose.read_text() if compose.is_file() else ""

    mode: ScaffoldMode | None = None
    if "Generated by `mimir scaffold-docker --mode pypi`" in docker_text:
        mode = "pypi"
    elif "Generated by `mimir scaffold-docker`" in docker_text:
        mode = "workspace"

    port_match = re.search(
        r'^\s*-\s*"127\.0\.0\.1:(\d+):8080"\s*$',
        compose_text,
        re.MULTILINE,
    )
    web_port = int(port_match.group(1)) if port_match else None
    service_match = re.search(
        r"^\s*container_name:\s*([^\s#]+)\s*$", compose_text, re.MULTILINE
    )
...
def _opencode_build_config_block(install_opencode: bool) -> str:
    ...
    return (
        ...
        "COPY --chown=mimir:mimir .config/opencode/opencode.json "
        "/home/mimir/.config/opencode/opencode.json"
    )
...
def scaffold(
    ...
    *,
    ...
    old_mode, old_port, old_service_name, old_install_opencode = (
        _existing_scaffold_settings(home)
    )
    mode = mode or old_mode or _DEFAULT_MODE
    web_port = web_port if web_port is not None else old_port or 8090
    install_opencode = (
        install_opencode
        if install_opencode is not None
        else old_install_opencode or False
    )
    if mode not in _MODES:
        raise ValueError(f"unknown scaffold mode {mode!r}; expected one of {_MODES}")

    if service_name is None:
        # Preserve a generated deployment's name; derive one only on first run.
        service_name = old_service_name or _sanitize_service_name(
            home.name or "mimir-agent"
        )
    else:
    ...
    if new_env != (existing or ""):
        if existing is None:
            fd = os.open(ce_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
            with os.fdopen(fd, "w") as compose_env:
                compose_env.write(new_env)
...
def add_argparse(parser) -> None:
    ...
    parser.add_argument(
        ...
        help="Host port to bind the agent's HTTP UI on "
             "(127.0.0.1:<port>). Default: preserve the existing scaffold, "
             "or 8090 on first run. Must be in 1–65535.",
    )
    parser.add_argument(
        "--mode", choices=list(_MODES), default=None,
        help=(
            "Deployment shape. Omit to preserve an existing scaffold; "
            "new deployments default to ``workspace``. ``workspace`` "
...
def cmd(args) -> int:
    ...
    if web_port is not None and not (1 <= web_port <= 65535):
        print(
            f"--web-port {web_port} out of range; must be 1–65535."
        )
        return 2
    ...
    try:
        result = scaffold(
            ...
            install_opencode=getattr(args, "opencode", None),
        )
