...
def test_render_dockerfile_omits_opencode_by_default():
    ...
    for mode in ("workspace", "pypi"):
        out = render_dockerfile([], mode=mode)
        assert "opencode-ai@1.18.9" not in out, mode
...
def test_scaffold_installs_opencode_when_flag_set(tmp_path: Path):
    ...
    assert "opencode-ai@1.18.9" in (home / "Dockerfile").read_text()

    plain = tmp_path / "plain-home"
    plain.mkdir()
    scaffold(plain, mode="pypi", install_opencode=False)
    assert "opencode-ai@1.18.9" not in (plain / "Dockerfile").read_text()
...
def test_cli_exposes_opencode_opt_in(tmp_path: Path, capsys):
    ...
    assert cmd(args) == 0
    assert "opencode-ai@1.18.9" in (tmp_path / "Dockerfile").read_text()
...
def test_cmd_flagless_rerun_preserves_deployment_shape(tmp_path: Path, capsys):
    ...
    assert "opencode-ai@1.18.9" in dockerfile
...
def test_explicit_rerun_flags_override_recovered_shape(tmp_path: Path):
    ...
    assert "opencode-ai@1.18.9" not in dockerfile
