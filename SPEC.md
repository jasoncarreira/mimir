...
- **Priority-banded suppression.** Before each fire the scheduler consults the homeostat (`HomeostaticArbiter.should_fire(priority=...)`), which grades resource pressure into CLEAR / ELEVATED / TIGHT / BLOCKED — under subscription billing from the per-window **burst multiple** M = (1−util)/(pace×time_left) with an early-window confidence ramp, under API billing from cost-rate trips, and BLOCKED from a recorded 429 pause. The pace bands engage only when the projection exceeds the 0.75 pace floor — a window not even projected near its cap has no quota story, regardless of M. Raw utilization ≥ 0.90 is a TIGHT wall, demoted to ELEVATED when the same window's pace shows coasting (M ≥ 2γ); without pace evidence (pegged/derived windows) it stays TIGHT. `low`-priority work sheds at ELEVATED, `normal` at TIGHT, `high` only at BLOCKED. A shed fire skips the subprocess (cursor frozen — events delayed, not lost) and emits `poller_fire_suppressed`. Scheduled ticks ride the same gate via `SchedulerJob.priority` (default `normal`, overridable per scheduler.yaml entry; explicit `low` remains available for disposable maintenance).
...
30. **S5 has no runtime S3/S4 arbitration mechanism — and doesn't need one.** Beer's VSM expects S5 to arbitrate when S3 (here-and-now control) and S4 (there-and-then intelligence) conflict — the canonical example being "an urgent user message arrives during a multi-step heartbeat; which wins?" The VSM eval (2026-05-23) flagged this as a viability gap because mimir has no such arbitration code. **The gap is mis-categorized: mimir's architecture prevents the S3/S4 conflicts that S5 arbitration would resolve.** Per-channel queues + the global semaphore (`MIMIR_MAX_CONCURRENT_TURNS`, default 10) mean `scheduler:heartbeat` and `discord-123` are separate channels that acquire turn slots independently. A user_message landing during a heartbeat just runs in parallel — no preemption needed, no priority decision, no policy arbitration required. The remaining real conflicts that DO exist between concurrent turns are already handled by other mechanisms: cross-scheduler file races by the S2-3 mutex (`Dispatcher._scheduler_tick_lock`, see §4.5), `memory/core/` writes by the S5-2 reflection-only gate (`WriteGuardBackend`), resource pressure by the homeostat (`budget.HomeostaticArbiter`), and per-channel ordering by the per-channel queue. None of these are "S5 should arbitrate" — they're S2/S3 mechanisms. Beer's framework assumes a single-track regulatory loop where conflict resolution is a real concern; mimir is multi-track by design, so the conflict class S5 arbitration would resolve doesn't arise. Operators reading the VSM vocabulary should NOT expect a code-level S5 arbitration policy — the architecture is multi-track-concurrent, not serialized-with-priority. *Full gap analysis: `state/wiki/topics/mimir-vsm-eval.md` GAP [S5-3].*
