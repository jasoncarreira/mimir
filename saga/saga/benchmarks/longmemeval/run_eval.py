...
Usage:
    ...
    python -m saga.benchmarks.longmemeval.run_eval            # full 500
...
def _prepare_environment(work_dir: Path, config_path: Path | None = None):
    ...
    cfg_path = Path(config_path) if config_path else BENCH_SAGA_CONFIG
    os.environ["SAGA_CONFIG"] = str(cfg_path)
    os.environ["SAGA_DATA_DIR"] = str(work_dir)
