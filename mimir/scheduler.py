...
class Scheduler:
    ...
    def add_quota_poll_job(
        ...
        *,
        model: str | None = None,
        job_id: str = "quota-poll",
    ) -> bool:
        """Periodic poll of Claude Max plan window utilization.

        ``ClaudeSDKClient.get_context_usage()`` exposes per-window
        utilization that doesn't make it through the message-stream
        events on Max OAuth. This cron spins up a throwaway client,
        queries the daemon, parses ``apiUsage``, and writes
        ...
        async def _poll() -> None:
            from .quota_poller import poll_max_plan_quota
            try:
                await poll_max_plan_quota(home, rate_limit_store, model=model)
            except Exception as exc:  # noqa: BLE001
                # poll_max_plan_quota already logs its own failures
                # via quota_poll_failed; this is just defense in depth
                # so a misbehaving client doesn't break the scheduler.
                log.exception("quota_poll job raised: %s", exc)
