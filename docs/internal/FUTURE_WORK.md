...
- **Indexer skips** — search + state `INDEX.md` generation now share `mimir/index_skip.py`: framework defaults skip `state/heartbeat-backlog.md`, `state/proposed-changes.md`, `state/identities.yaml`, `state/pollers/`, and `state/social/`. Deployment-local experiment/workspace skips belong in `<home>/.mimir/index-skip.txt`, one home-relative path per line (comments with `#` allowed); entries match either the exact file or descendants on a path boundary, so `state/social` skips `state/social/...` but not `state/social-notes.md`. Operators upgrading from old hardcoded local experiment skips should add entries such as `state/openclaw-tools` and `state/hermes-npm-inspect` if those trees should remain out of file_search/state indexes. Resolves the "social-cli artifacts shouldn't be embedded" tail of §10.1.
