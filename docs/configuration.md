...
| `MIMIR_FILE_TOOL_ROOTS` | csv `/absolute/path[:ro\|:rw]` | `""` | Sole behavior-bearing configuration for roots beyond home; comma separates entries and colon only delimits the optional mode. External roots are routed to all file policies and tools; `/tmp` is derived as `rw`. Invalid entries warn with their value, reason, and expected form. Rejects traversal, missing directories, denylisted system roots, and home overlap. Effective roots and their configured/derived origin are logged at startup. See [file-tool access](../README.md#file-tool-access-outside-the-home). |
