...
def test_reaper_ttl_floor_is_dimensioned_by_leaf_timeout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("MIMIR_FACTORY_RUN_TIMEOUT_S", "5000.5")

    with pytest.raises(ValueError) as excinfo:
        WorklinkDefaults(timeout_s=1800, reaper_ttl_s=3599)

    assert excinfo.value.configured_value == 3599
    assert excinfo.value.required_value == 3600
    assert "configured 3599; required 3600" in str(excinfo.value)
    assert WorklinkDefaults(timeout_s=1800, reaper_ttl_s=3600).reaper_ttl_s == 3600
...
def _openrouter_env(tmp_path: Path, **values: str) -> dict[str, str]:
    """Env for a Worklink resolution that would select OpenRouter."""
    home = tmp_path / "home"
...
def test_worklink_resolution_refuses_prohibited_provider(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, credential: str,
) -> None:
    """OpenRouter is refused whichever credential transport supplies it.

    Both routes reach the worker through the projected auth document rather
    than the worker env allowlist, so filtering env alone would not stop them.
    """
    ...
    env = _openrouter_env(tmp_path)
    if credential == "saved_auth":
        _write_opencode_auth(tmp_path, {"openrouter": {"type": "api", "key": "sk-or-saved"}})
    else:
        env["OPENROUTER_API_KEY"] = "sk-or-ambient"

    with pytest.raises(OpenCodeConfigError) as excinfo:
        resolve_worklink_opencode_invocation(env)
    assert excinfo.value.reason_code == "config_provider_selection"


def test_factory_resolution_refuses_prohibited_provider(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The factory path funnels through the same boundary and surfaces it typed."""
    from mimir.worklink.backends.feature_factory import FactoryContractError

    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    _write_opencode_auth(tmp_path, {"openrouter": {"type": "api", "key": "sk-or-saved"}})
    backend = FeatureFactoryBackend(entrypoint="/opt/factory/bin/factory.js")
    order = WorkOrder(
        4242, tmp_path, "prompt", None, 30, env=_openrouter_env(tmp_path),
    )
    ...
    assert "feature_factory_opencode_resolution_failed:config_provider_selection" in str(
        excinfo.value
    )


def test_worklink_resolution_admits_a_permitted_provider(tmp_path: Path) -> None:
    """The prohibition is provider-scoped, not a blanket refusal."""
    from mimir.worklink.backends.opencode import resolve_worklink_opencode_invocation

    _write_opencode_auth(tmp_path, {"zai": {"type": "api", "key": "zai-key"}})
    env = _openrouter_env(tmp_path, MIMIR_MODEL_SPEC="zai:glm-5")
