...
1. **Pass 1 — dedup.** Tight-threshold clustering (**0.92 floor for all providers** — computed as `max(_PROVIDER_AUTO_THRESHOLDS[provider], 0.92)`, so providers whose thematic threshold is below 0.92 like openai / nim get bumped up to 0.92 for dedup) collapses near-duplicate raws into a canonical chosen by **ACT-R activation** (Petrov OL — see `mimir/saga/activation.py`), with tiebreaks pinned > observation trend > evidence_count > older-created. The floor is calibrated against mimir's saga.db where Voyage 0.92 and OpenAI 0.92 both sit at the ~99.98th percentile of pair similarity. Duplicates are tombstoned with `reason='merged'`; their `access_events` are redirected to the canonical (activation history preserved by sum-linearity), their `atom_relations` are redirected with dedup, and a `consolidated_into` edge is added. No LLM cost.
2. **Pass 2 — thematic.** The original observation-synthesis pass (default 0.80 voyage / 0.80 openai-NIM, per-provider via `_PROVIDER_AUTO_THRESHOLDS`) runs on the now-deduped candidate set and emits one observation per surviving cluster.
