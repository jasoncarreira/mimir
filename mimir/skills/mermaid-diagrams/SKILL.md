...
<!-- desc: Comprehensive guide for creating software diagrams using Mermaid syntax — class, sequence, flowchart, ER, C4, state, git, gantt. -->
...
```bash
# 1. Write the mmd source under the outbound attachments dir.
mkdir -p "$MIMIR_HOME/attachments/outbound"
cat > "$MIMIR_HOME/attachments/outbound/architecture.mmd" <<'EOF'
flowchart LR
    A[Client] --> B[API]
    B --> C[(DB)]
EOF

# 2. Render to PNG in the same dir (so <send-file> can resolve it).
mmdc -i "$MIMIR_HOME/attachments/outbound/architecture.mmd" \
     -o "$MIMIR_HOME/attachments/outbound/architecture.png"
```

Then send via the directive — single `send_message` call:

```
Here's the shape I have in mind.

<actions>
  <send-file path="architecture.png" kind="image" caption="Architecture sketch" cleanup="true" />
</actions>
```

`cleanup="true"` deletes the rendered PNG after a successful send so
the attachments dir doesn't accumulate stale renders.
