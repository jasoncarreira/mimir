...
class _Output:
    ...
    def write(self, data: bytes) -> None: self.stream.write(data); self.stream.flush()
...
def _socket(home: Path) -> Path:
    ...
    uid = os.getuid()
    if not stat.S_ISSOCK(value.st_mode) or stat.S_ISLNK(value.st_mode) or value.st_uid != uid or not stat.S_ISDIR(parent.st_mode) or parent.st_uid != uid or parent.st_mode & 0o077:
