...
model_zoo = {
    ...
    # OpenRouter passes through any model id; auth via OPENAI_API_KEY env
    # set to the OpenRouter key, base URL via OPENAI_BASE_URL.
    ...
    with open(result_file, 'w') as out_f:
        ...
        for entry in tqdm(hypotheses):
            ...
            kwargs = {
                ...
                'n': 1,
                'temperature': 0,
                # 10 tokens is enough for gpt-4o's "yes"/"no" but reasoning
                # models (gpt-oss, o1, deepseek-r1) need budget for the
                # internal CoT before emitting the answer. 256 leaves
                # plenty of headroom; non-reasoning models stop early.
                'max_tokens': 256
            }
            completion = chat_completions_with_backoff(metric_client, **kwargs)
            # Reasoning models (gpt-oss, o1, deepseek-r1, ...) may put their
            # output in `message.reasoning` and leave `message.content` None.
            # Fall back to reasoning text in that case.
            _msg = completion.choices[0].message
            _content = getattr(_msg, 'content', None) or getattr(_msg, 'reasoning', None) or ''
            eval_response = _content.strip()
            # max_tokens=10 can also truncate before the model emits "yes"/"no".
            # If the reasoning ran long but no decision word landed, default
            # to "no" — better to under-credit than to falsely credit.
