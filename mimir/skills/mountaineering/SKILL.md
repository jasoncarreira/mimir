...
In mimir, the climber is a bundled subagent (see
`~/.claude/agents/climber.md`). Fan out a climb by invoking the SDK's
`Task` / `Agent` tool with `subagent_type=climber` and
`background=true` for long-running climbs; the parent supervises by
polling the subagent inbox or waiting on `TaskNotificationMessage`.
The climber reads `<climb_dir>/program.md` + `<climb_dir>/score.sh`,
writes its sliding-window log to `<climb_dir>/log.jsonl`, and lands a
final `<climb_dir>/result.md` when it stops.

The climber inherits whatever skills and tools mimir already has
configured — no separate setup. The discipline below (Five Laws,
propose-test-keep, scope, when to stop) is identical to open-strix's
mountaineering; only the mechanism differs.
