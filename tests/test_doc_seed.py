...
def test_seed_and_refresh_publish_changed_docs(source, tmp_path):
    home = _home(tmp_path)
    ...
    assert statuses["docs/configuration.md"] == "present"
...
def test_restore_replaces_preexisting_content(source, tmp_path):
    home = _home(tmp_path)
    existing = home / "docs/configuration.md"
    existing.parent.mkdir()
