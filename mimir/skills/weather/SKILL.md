...
success_criteria:
  ...
  any_of:
    - tool_call:
        name: Bash
        args:
          command_glob: "*mimir.skills.weather.scripts.get_weather*"
    - tool_call:
        name: Bash
        args:
...
Always invoke via Python module syntax — `python3 -m mimir.skills.weather.scripts.get_weather`.
...
`python3 mimir/skills/weather/scripts/get_weather.py` only resolve from a
...
correctly installed, `python3 -c "import mimir.skills.weather.scripts.get_weather"`
should exit 0.

```bash
# Default location (set inside the script — Victor, NY, US).
python3 -m mimir.skills.weather.scripts.get_weather

# Specific city. Comma-separated; ISO country codes for disambiguation.
python3 -m mimir.skills.weather.scripts.get_weather "London,UK"
python3 -m mimir.skills.weather.scripts.get_weather "San Francisco,CA,US"
```

The script does NOT use argparse — anything that isn't `--json` is
treated as the city name. So `python3 -m mimir.skills.weather.scripts.get_weather --help`
...
```bash
python3 -m mimir.skills.weather.scripts.get_weather "Tokyo,JP" --json
```
