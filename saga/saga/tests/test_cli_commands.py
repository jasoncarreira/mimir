...
def metrics_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("saga.metrics.METRICS_DB", mdb)
    return mdb
...
class TestTripleStats:
    def test_triple_stats_output(self, capsys, monkeypatch):
        ...
        init_triples_schema(conn)
        conn.close()
        monkeypatch.setattr("saga.remember.log_triple_store_snapshot", lambda: None)
        from saga.remember import cmd_triple_stats
        cmd_triple_stats()
...
class TestMetamemory:
    def test_metamemory_no_args(self, capsys):
        from saga.remember import cmd_metamemory
        cmd_metamemory([])
        data = json.loads(capsys.readouterr().out)
...
class TestDrift:
    def test_drift_no_args(self, capsys):
        from saga.remember import cmd_drift
        cmd_drift([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestConfidenceDecay:
    def test_confidence_decay_runs(self, capsys):
        ...
        from saga.remember import cmd_confidence_decay
...
class TestSessionClear:
    def test_session_clear_runs(self, capsys):
        from saga.remember import cmd_session_clear
        cmd_session_clear()
        data = json.loads(capsys.readouterr().out)
...
class TestCache:
    def test_cache_stats(self, capsys):
        from saga.remember import cmd_cache
        cmd_cache([])
        data = json.loads(capsys.readouterr().out)
        assert isinstance(data, dict)
...
class TestAnalytics:
    def test_analytics_runs(self, capsys, metrics_db):
        from saga.remember import cmd_analytics
        cmd_analytics([])
        data = json.loads(capsys.readouterr().out)
        assert isinstance(data, dict)
...
class TestMigrate:
    def test_migrate_runs(self, capsys):
        from saga.remember import cmd_migrate
        cmd_migrate([])
        data = json.loads(capsys.readouterr().out)
        assert isinstance(data, dict)
...
class TestForgettingCmd:
    def test_forgetting_no_args(self, capsys):
        from saga.remember import cmd_forgetting
        cmd_forgetting([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestGaps:
    def test_gaps_no_args(self, capsys):
        from saga.remember import cmd_gaps
        cmd_gaps([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestVersions:
    def test_versions_no_args(self, capsys):
        from saga.remember import cmd_versions
        cmd_versions([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestOutcomesCmd:
    def test_outcomes_summary(self, capsys):
        from saga.remember import cmd_outcomes
...
class TestEmotional:
    def test_emotional_file_not_found(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.remember.WORKSPACE", "/nonexistent")
        from saga.remember import cmd_emotional
        cmd_emotional([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
    ...
    def test_emotional_parses_file(self, capsys, monkeypatch, tmp_path):
        ...
        ws = str(tmp_path)
        monkeypatch.setattr("saga.remember.WORKSPACE", ws)
        ...
        monkeypatch.setattr("saga.metrics.log_emotional_state",
...
class TestDryCmd:
    def test_dry_no_args(self, capsys):
        from saga.remember import cmd_dry
        cmd_dry([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestRewriteCmd:
    def test_rewrite_no_args(self, capsys):
        from saga.remember import cmd_rewrite
        cmd_rewrite([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestDiverse:
    def test_diverse_no_args(self, capsys):
        from saga.remember import cmd_diverse
        cmd_diverse([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestEmotionRetrieve:
    def test_emotion_retrieve_no_args(self, capsys):
        from saga.remember import cmd_emotion_retrieve
...
class TestNegative:
    def test_negative_no_args(self, capsys):
        from saga.remember import cmd_negative
        cmd_negative([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestAssociations:
    def test_associations_no_args(self, capsys):
        from saga.remember import cmd_associations
        cmd_associations([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestGraph:
    def test_graph_no_args(self, capsys):
        from saga.remember import cmd_graph
        cmd_graph([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data

    def test_graph_entity(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.triples.graph_traverse",
    ...
    def test_graph_path(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.triples.graph_path",
...
class TestConfidence:
    def test_confidence_runs(self, capsys):
        ...
        from saga.remember import cmd_confidence
        cmd_confidence([])
        data = json.loads(capsys.readouterr().out)
        assert isinstance(data, dict)
...
class TestContribute:
    def test_contribute_no_args(self, capsys):
        from saga.remember import cmd_contribute
        cmd_contribute([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestFeedbackMark:
    def test_feedback_mark_no_args(self, capsys):
        from saga.remember import cmd_feedback_mark
        cmd_feedback_mark([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestSessionBoundary:
    def test_session_boundary_list(self, capsys):
        from saga.remember import cmd_session_boundary
...
class TestPin:
    def test_pin_list(self, capsys):
        from saga.remember import cmd_pin
        cmd_pin([])
        data = json.loads(capsys.readouterr().out)
...
class TestMerge:
    def test_merge_candidates(self, capsys):
        ...
        from saga.remember import cmd_merge
...
class TestImportance:
    def test_importance_no_args(self, capsys):
        from saga.remember import cmd_importance
        cmd_importance([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestForget:
    def test_forget_dry_run(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.forgetting.identify_forgetting_candidates",
                            lambda dry_run=True: {"candidates": [], "dry_run": dry_run})
        from saga.remember import cmd_forget
        cmd_forget(["--dry-run"])
        data = json.loads(capsys.readouterr().out)
...
class TestSplit:
    def test_split_no_args(self, capsys):
        from saga.remember import cmd_split
        cmd_split([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestBatch:
    def test_batch_queries(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.core.batch_query", lambda qs: [
            {"query_type": "mixed", "triples": [], "atoms": [], "total_tokens": 10}
            for _ in qs
        ])
        from saga.remember import cmd_batch
...
class TestExplain:
    def test_explain_no_args(self, capsys):
        from saga.remember import cmd_explain
        cmd_explain([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestProvenance:
    def test_provenance_no_args(self, capsys):
        from saga.remember import cmd_provenance
        cmd_provenance([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestSummarize:
    def test_summarize_no_args(self, capsys):
        from saga.remember import cmd_summarize
        cmd_summarize([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data
...
class TestCalibrate:
    def test_calibrate_no_args(self, capsys):
        from saga.remember import cmd_calibrate
        cmd_calibrate([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data

    def test_calibrate_with_provider(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.calibration.calibrate",
...
class TestReembed:
    def test_reembed_no_args(self, capsys):
        from saga.remember import cmd_reembed
        cmd_reembed([])
        data = json.loads(capsys.readouterr().out)
        assert "error" in data

    def test_reembed_dry_run(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.calibration.re_embed",
...
class TestWorldCmd:
    def test_world_show_all(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.triples.query_world", lambda **kw: [])
        from saga.remember import cmd_world
        cmd_world([])
        data = json.loads(capsys.readouterr().out)
    ...
    def test_world_entity_query(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.triples.query_world",
    ...
    def test_world_set(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.triples.update_world",
    ...
    def test_world_history(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.triples.world_history",
...
class TestPredictCmd:
    def test_predict_context_mode(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.prediction.PredictiveEngine.predict_context",
                            lambda self, hour=None, day_of_week=None, top_k=8: [])
        from saga.remember import cmd_predict
    ...
    def test_predict_learn(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.prediction.PredictiveEngine.learn_from_session",
    ...
    def test_predict_default(self, capsys, monkeypatch):
        monkeypatch.setattr("saga.core.predict_needed_atoms", lambda ctx: [])
        from saga.remember import cmd_predict
...
class TestFeedbackCmd:
    def test_feedback_analyze(self, capsys):
        from saga.remember import cmd_feedback
