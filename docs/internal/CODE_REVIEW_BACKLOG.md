...
- **Status:** ✅ Landed 2026-05-07. The `call_later` callback now spawns the `_fire_idle` task and synchronously swaps `session.idle_handle` from the spent TimerHandle to the in-flight Task, so any subsequent `touch()` / `end_now()` / `shutdown()` `cancel()` reaches the actual task. The fire callback also bails if the session has been replaced or ended between scheduling and firing. Two regression tests pin the post-fire-pre-task-completion window and the replaced-session no-op path.
...
- **Status:** ✅ Landed 2026-05-07. The four `agent.run_turn` walks (ResultMessage capture, RateLimit/StreamEvent handling, TaskStarted log+description, TaskProgress/Notification handling) are now a single dispatch walk over `messages`. `task_descriptions` is filled by the Started branch and read by the Notification branch in the same pass — relies on the SDK's documented invariant that TaskStarted precedes any Progress/Notification for the same `task_id`. Plan-quota capture moved from between walks to after the consolidated walk (no ordering dependency either direction). One regression test pins the dispatch invariant against interleaved subagent streams. `extract_turn_events`'s internal 2-pass (tool_indices precompute + emit) was left alone — the second pass requires the precomputed `intermediate_indices` set, so consolidating it would require re-architecting the streaming-active path.
