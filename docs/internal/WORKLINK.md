...
- **Tool-pin inventory (external-toolchain drift).** `worklink.yaml`
  ...
  | claude-code | `coding-cli` | npm package `@anthropic-ai/claude-code` | `2.1.220` | root/scaffold Dockerfiles when `MIMIR_ENABLE_CLAUDE_CODE=1` | `claude --version` | Medium: optional second coding backend; real smoke requires a deployment with `claude` installed. |
