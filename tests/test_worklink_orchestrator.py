...
def test_finalize_primary_incident_survives_secondary_transition_failure(
    ...
    def secondary_failure(self: ChainlinkClaims, *args: object, **kwargs: object) -> None:
        raise RuntimeError("secondary label routing failure")
    ...
    monkeypatch.setattr(ChainlinkClaims, "transition_issue", secondary_failure)
    ...
    assert incident["terminal_error"] != "RuntimeError: secondary label routing failure"
    assert incident["attempt"] == 1
    assert incident["preserved_ref"] == "issue/441-a1"
    assert incident["work_path"] == str(worktree)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("concurrency cap reached (1/1 active claims)", "refused"),
        ("concurrency cap reached (4/3 active claims)", "refused"),
        ("lock service unavailable", "failed"),
        ("claim guard failed", "failed"),
        (None, "failed"),
    ],
)
def test_claim_refusal_dispatch_failure_accounting(
    tmp_path: Path,
    ...
    if epic:
        issue["labels"].append("worklink:epic")
    ...
    monkeypatch.setattr(orchestrator, "_runner_for_home", lambda *_: runner)
    monkeypatch.setattr(FeatureFactoryBackend, "admit", lambda self: Path(self.entrypoint))
    monkeypatch.setattr(ChainlinkClaims, "claim_issue", refuse_claim)
    monkeypatch.setattr(
    ...
    for consecutive in (1, 2, 3):
        ...
        if status == "refused":
            ...
            backed_off, alerts = pending_failure_alerts(
                state_dir, now=datetime.fromisoformat(entry["failed_at"])
            )
...
def test_push_failure_blocks_build_and_reports_publication_step(tmp_path: Path) -> None:
    ...
    def runner(
        ...
        *,
        ...
        return base_runner(args, cwd=cwd, text=text, timeout=timeout)
...
def test_publication_replacement_interleaving(tmp_path: Path, monkeypatch, fenced, crash) -> None:
    """A distinct controller tries admission after remote creation, before evidence.
    ...
    def run_attempt(attempt):
        ...
        if attempt == 2 and fenced:
            assert result.status == "refused"
            assert result.reason == "publication_intent_exists"
            assert result.attempt is None
            assert result.checkout is None
            assert not worktree.exists()
            assert "worklink:ready" in issue["labels"]
            assert not any(
                isinstance(call, list) and (
                    call[:2] == ["chainlink", "locks"]
                    or call[:3] in [
            ...
            assert not (tmp_path / "state/worklink/evidence/441-2.json").exists()
        else:
    ...
    try:
        ...
        # Admission must refuse independently of reliable liveness.
        ...
        assert replacement.status == ("refused" if fenced else "completed")
        assert publications.value == (1 if fenced else 2)
        if fenced:
            assert replacement.reason == "publication_intent_exists"
        if not crash:
            finish.set()
            process.join(10)
...
def test_publication_fence_failures_and_retry(tmp_path: Path, monkeypatch, failure) -> None:
    ...
    if failure == "existing-intent":
        ...
        assert result.checkout is None
        assert not worktree.exists()
        assert not (evidence_dir / "441-1.json").exists()
        assert not any(
            isinstance(call, list) and (
                call[:2] == ["chainlink", "locks"]
                or call[:3] in [
    ...
    fail = False
    issue = json.loads(ISSUE_JSON)
    issue["comments"] = [] if failure == "existing-intent" else [ClaimRecord(
        441, 1, "original", datetime(2020, 1, 1, tzinfo=UTC),
    ).to_comment()]
    next_attempt = 1 if failure == "existing-intent" else 2
    worktree = worktree.with_name(f"441-{next_attempt}")
    calls, base_runner = _orchestrator_runner(repo, worktree, issue_json=json.dumps(issue))
    retried = asyncio.run(orchestrator.run(441, backend_name="fake", test_command="echo ok"))
    assert retried.status == ("refused" if retained else "completed"), retried.reason
    if retained:
        assert retried.reason == "publication_intent_exists"
        assert retried.attempt is None
        assert retried.checkout is None
        assert not worktree.exists()
        assert "worklink:ready" in issue["labels"]
        assert not (evidence_dir / f"441-{next_attempt}.json").exists()
        assert not any(
            isinstance(call, list) and (
                call[:2] == ["chainlink", "locks"]
                or call[:3] in [
    ...
    if failure in {"pr", "existing-intent"}:
        ...
        calls, base_runner = _orchestrator_runner(
            repo, worktree, issue_json=json.dumps(issue),
        )
        reconciled = asyncio.run(orchestrator.run(
            441, backend_name="fake", test_command="echo ok",
        ))
        assert reconciled.status == "completed", reconciled.reason
        assert reconciled.attempt == next_attempt
        assert ["chainlink", "locks", "claim", "441"] in calls
        assert ["git", "clone", "--local", "--quiet", str(repo), str(worktree)] in calls
        assert len(publications) == (2 if failure == "pr" else 1)
...
@pytest.mark.parametrize("failure_kind", ["result", "exception", "base-exception"])
def test_leaf_failure_is_durable_before_release_and_blocks_concurrent_core_admission(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, failure_kind: str,
) -> None:
    import mimir.worklink.orchestrator as orchestrator
    ...
    class FailingBackend(FakeBackend):
        async def interpret(self, order: WorkOrder, result: object) -> RawResult:
            if failure_kind == "exception":
                raise RuntimeError("interpret exploded")
            if failure_kind == "base-exception":
                raise KeyboardInterrupt("operator stop")
            return await super().interpret(order, result)
    ...
    def record_and_probe(**kwargs: object) -> dict[str, Any] | None:
        ...
        thread = threading.Thread(
            target=lambda: concurrent.append(
                run_worklink(
                    home=tmp_path, repo=repo, issue_id=441, autonomous=True
                )
            )
        )
        thread.start()
    ...
    def release_after_incident(self: ChainlinkClaims, issue_id: int) -> bool:
        assert order_seen == ["incident"]
        order_seen.append("release")
        return True

    monkeypatch.setattr(WorklinkConfig, "load", lambda *_: config)
    monkeypatch.setattr(orchestrator, "_record_run_failure", record_and_probe)
    monkeypatch.setattr(ChainlinkClaims, "release_issue", release_after_incident)
    ...
    if failure_kind == "base-exception":
        ...
        assert result.status == "failed"
        assert order_seen == ["incident", "release"]
...
def test_gate_timeout_blocks_instead_of_retrying(tmp_path: Path, timed_out: bool) -> None:
    ...
    def runner(args, **kwargs):
        if isinstance(args, str):
            calls.append(args)
            if timed_out:
                raise subprocess.TimeoutExpired(args, kwargs["timeout"], output=b"gate stalled")
            return cp(args, returncode=1, stdout="assertion failed\n")
        return base_runner(args, **kwargs)
    ...
    assert result.attempt == 1
    assert result.status == ("blocked" if timed_out else "failed")
    assert not result.review_ready
    assert result.pr_url is None
    assert (["chainlink", "issue", "label", "441", "worklink:blocked"] in calls) is timed_out
    assert (["chainlink", "issue", "label", "441", "worklink:ready"] in calls) is (not timed_out)
    assert ["chainlink", "locks", "release", "441"] in calls
    assert not any(isinstance(call, list) and call[:3] == ["gh", "pr", "create"] for call in calls)
    evidence = json.loads(result.evidence_path.read_text(encoding="utf-8"))
    assert evidence["tests"]["timed_out"] is timed_out
    reason = "gate_timed_out" if timed_out else "tests_failed"
    assert any(
        isinstance(call, list) and call[:3] == ["chainlink", "issue", "comment"]
...
@pytest.mark.parametrize("files_changed", [[], ["legitimate.py"]], ids=["empty-diff", "legitimate-diff"])
@pytest.mark.parametrize("root_state", ["escaped", "clean", "inside", "preexisting"])
def test_outside_checkout_detection_is_independent_of_attempt_diff(
    tmp_path: Path, files_changed: list[str], root_state: str,
) -> None:
    from mimir.worklink.orchestrator import _with_outside_checkout_detection

    checkout = tmp_path / "attempt"
    status = "completed" if files_changed else "failed"
    reasons = () if files_changed else ("completed_empty_diff",)
    validation = EvidenceValidation(
        status=status,
        review_ready=bool(files_changed),
        reasons=reasons,
        evidence=WorklinkEvidence(
            issue=517, attempt=1, backend="codex", branch="issue/517-a1",
            checkout=str(checkout), started_at="2026-06-16T20:00:00+00:00",
            finished_at="2026-06-16T20:05:00+00:00", files_changed=files_changed,
            diff_stat="", commands=[], tests=None, pr_url=None, status=status,
        ),
    )
    output = {
        "escaped": " M leaked.py\n",
        "clean": "",
        "inside": " M attempt/legitimate.py\n",
        "preexisting": " M operator.py\n",
    }[root_state]
    calls: list[list[str]] = []

    def runner(args: Sequence[str]) -> subprocess.CompletedProcess[str]:
        calls.append(list(args))
        return subprocess.CompletedProcess(args, 0, stdout=output if args[3] == "status" else "", stderr="")

    result = _with_outside_checkout_detection(
        validation, issue=517, attempt=1, root=tmp_path, checkout=checkout,
        runner=runner, root_dirty_before=("operator.py",),
    )

    assert calls[0] == [
        "git", "-C", str(tmp_path), "status", "--porcelain=v1", "--untracked-files=all",
    ]
    if root_state == "escaped":
        assert result.status == "failed"
        assert result.review_ready is False
        assert result.evidence.status == "failed"
        assert result.evidence.files_changed == files_changed
        assert result.reasons == (*reasons, (
            "backend_wrote_outside_checkout: leaked.py "
            "(quarantined to git stash 'worklink-leak-517-a1' in the repo root)"
        ))
        assert calls[1:] == [[
            "git", "-C", str(tmp_path), "stash", "push", "--include-untracked",
            "-m", "worklink-leak-517-a1", "--", "leaked.py",
        ]]
    else:
...
def _run_factory_preflight_case(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        "release_issue",
        lambda *args, **kwargs: (
            on_release() if on_release is not None else None
        )
        or release_confirmed,
    )
        ...
        async def supervise(*args: object, **kwargs: object) -> object:
            ...
            if outcome == "post_merge_refusal":
                ...
                status = parse_factory_status({
                    ...
                    "gates": {
                        "pre_pr": {
                            "status": "running", "at": None, "artifact": None,
                            "reviewed_head": None,
                        },
                    },
...
def test_factory_start_prompt_at_durable_launch(tmp_path, monkeypatch, autonomous, launched):
    ...
    async def terminal(**kwargs):
        assert load_factory_record(tmp_path, "chainlink-700").handle is not None
    ...
    assert result.status == ("blocked" if launched else "failed")
    assert [entry["kind"] for entry in entries()] == (
        ["factory_start"] if autonomous and launched else []
    )
    if autonomous and launched:
        assert entries()[0]["issue_title"] == "epic"
...
def test_factory_success_prompt_only_after_success(tmp_path, monkeypatch, autonomous, ending):
    ...
    if autonomous and ending == "completed":
        assert entries[1] == {
            "kind": "factory_success", "issue_id": 700, "issue_title": "epic", "run_id": "chainlink-700",
            "attempt": 1, "pr_url": pr_url, "notified": False,
            "delivery_key": "worklink-factory_success:700:chainlink-700:1",
        }


@pytest.mark.parametrize("failed_kind", ["factory_start", "factory_success"])
def test_factory_milestone_write_failure_preserves_result(tmp_path, monkeypatch, failed_kind):
    import mimir.worklink.orchestrator as orchestrator
    ...
    logged = []
    attempted = []
    expected = WorklinkRunResult(700, 1, "review_ready", pr_url="https://github.com/owner/repo/pull/42")
    record = dispatch_failures.record_factory_transition

    async def cleanup(compute, handle):
        cleaned.append(handle)

    def fail(state_dir, **kwargs):
        attempted.append(kwargs["kind"])
        if kwargs["kind"] == failed_kind:
            raise OSError("transition write failed")
        return record(state_dir, **kwargs)
    ...
    monkeypatch.setattr(orchestrator, "_log_event", lambda event, **kw: logged.append((event, kw)))
    result, launched, _, _ = _run_factory_preflight_case(
        tmp_path, monkeypatch, credentials={"GITHUB_TOKEN": "github-token"},
        autonomous=True, outcome="completed", terminal=terminal,
    )
    assert len(launched) == 1
    assert cleaned == []
    assert result is expected
    assert attempted == ["factory_start", "factory_success"]
    failures = [kw for event, kw in logged if event == "worklink_factory_milestone_write_failed"]
    assert len(failures) == 1
    assert failures[0]["kind"] == failed_kind
    assert "transition write failed" in failures[0]["error"]
...
@pytest.mark.parametrize("interrupted", [False, True], ids=["failure", "interruption"])
def test_factory_failure_is_durable_before_release_and_blocks_concurrent_core_admission(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, interrupted: bool,
) -> None:
    import mimir.worklink.orchestrator as orchestrator
...
def test_unbindable_factory_record_blocks_before_claim_and_is_preserved(
    ...
    elif refusal in {"sandbox", "sandbox_secondary_read"}:
        (old_sandbox / ".git").rmdir()
        old_sandbox.rmdir()
    ...
    def runner(args: Sequence[str] | str, **_: object) -> subprocess.CompletedProcess[str]:
        nonlocal issue_reads, remote_reads
        if isinstance(args, list) and args[:4] == ["chainlink", "issue", "show", "700"]:
            issue_reads += 1
            if refusal == "sandbox_secondary_read" and issue_reads > 2:
                raise RuntimeError("secondary retained issue read failed")
            return cp(args, stdout=epic)
        if isinstance(args, list) and args[:4] == ["git", "-C", str(repo), "config"]:
            remote_reads += 1
    ...
    if refusal == "run":
        monkeypatch.setattr(
            orchestrator, "factory_record_run_ids", lambda issue_id: ("other", "ids")
        )
    if refusal == "ownership_boundary":
    ...
    if refusal == "sandbox_secondary_read":
        with pytest.raises(RuntimeError, match="secondary retained issue read failed"):
            run_worklink_epic(
                home=tmp_path, repo=repo, issue_id=700, autonomous=True
            )
        result = None
    else:
        result = run_worklink_epic(
    ...
    expected_reasons = {
        ...
        "run": "retained factory issue identity does not match recovery request",
        ...
        "ownership_boundary": "legacy factory checkout has no private ownership-transfer boundary",
        "sandbox": "retained factory sandbox is unavailable",
        "sandbox_secondary_read": "retained factory sandbox is unavailable",
    ...
    assert old_sandbox.is_dir() is (
        refusal not in {"sandbox", "sandbox_secondary_read"}
    )
    from mimir.worklink.dispatch_failures import dispatch_failure_state_dir, load_failure_state
    ...
    assert "secondary retained issue read failed" not in incident["terminal_error"]
...
def test_factory_stale_status_events_are_bounded_per_episode(
    ...
    class Backend:
        ...
        def status(self, *args: object, **kwargs: object) -> Any:
            ...
            if factory_clock.now == 4000 and ending in {"parked", "newer-incident"}:
    ...
    if ending == "newer-incident":
        import mimir.worklink.dispatch_failures as dispatch_failures

        real_resolve = dispatch_failures.resolve_failure_if_current
        resolve_calls = 0
        ...
        def race_newer(
            ...
            if resolve_calls == 2:
                dispatch_failures.record_failure(
                    ...
                    attempt=2,
                    exit_status=1,
                    error="newer terminal factory failure",
                    log_path="newer.log",
                )
            return real_resolve(state_dir, issue_id, signature, occurrence_id)

        monkeypatch.setattr(
            dispatch_failures, "resolve_failure_if_current", race_newer
        )
    backend = Backend()
    with (
    ...
    if ending == "parked":
        assert incident["active"] is False
        assert pending_failure_alerts(dispatch_failure_state_dir(tmp_path)) == (set(), [])
    elif ending == "newer-incident":
        assert incident["active"] is True
        assert incident["terminal_error"] == "newer terminal factory failure"
...
def test_factory_completion_with_real_evidence_and_wrapped_runner(tmp_path: Path) -> None:
    ...
    def controller(args, cwd=None, *, text=True, timeout=1800):
        if isinstance(args, str):
...
@pytest.mark.parametrize("options", [{}, {"timeout": 7}])
def test_factory_git_runner_forwards_controller_options(tmp_path: Path, args, text: bool, options) -> None:
    ...
    def controller(args, cwd=None, *, text=True, **kwargs):
        calls.append((args, cwd, text, kwargs))
        return result

    assert _factory_git_runner(controller)(args, tmp_path, text=text, **options) is result
    assert calls == [(args, tmp_path, text, options)]
...
def test_factory_recovery_launch_start(
    ...
    expected = {
        "kind": "factory_start", "issue_id": 700, "issue_title": "epic",
        "run_id": "700", "attempt": 3,
        "pr_url": None, "delivery_key": key, "notified": existing_start,
    }
    if existing_start:
        failures.record_factory_transition(
            state_dir, kind="factory_start", issue_id=700, issue_title="epic", run_id="700", attempt=3,
        )
    ...
    async def supervise(self: object, **kwargs: Any) -> str:
        ...
        assert transitions == (
            {key: expected} if (autonomous or existing_start) and write_failure != "transition" else {}
        )
        return "supervised"
    ...
    if write_failure == "record":
        ...
        assert saved.handle == retained.handle
    else:
