...
The scheduled-maintenance shell profile rejects pipelines, redirects, `&&`,
and globs. Prefer `read_file`, `ls`, `glob`, `grep`, and `file_search` for file
inspection; reserve direct shell commands for the tightly admitted `git`, `gh`,
and `chainlink` queries that structured tools cannot express. The maintenance
profile does not admit the `mimir` CLI; use the report artifacts produced by its
...
- **Introspection report** — read the newest
  `state/reports/introspection-YYYY-MM-DD.md` generated by the dedicated weekly
  in-process `introspection-report` job. It precomputes the structured shape
  work that used to be hand-rolled `jq` queries — turn counts by trigger, tool
  error rates, recurring errors, behavioral drift week-over-week, heartbeat
  pipeline health, performance trends, and skill invocation counts. The job
  also emits `heartbeat_health_degraded` when scheduled-tick pipeline success
  falls below 80%. Use `read_file`/`grep` spot checks for things the report does
  not cover rather than raw shell `jq` pipelines. If the report is missing or
  stale, record that as a scheduler-health finding; do not invoke the `mimir`
  CLI from this maintenance-profile turn.
- `logs/events.jsonl` — every tool call, denial, error, scheduler
  event. Filter to last 7 days with `read_file`/`grep` spot checks after using
  the introspection report for aggregate shape work.
...
- **Recent session boundaries** — the prompt's "Recent session
  ...
  at all.)
- **Most-retrieved atoms** — use SAGA retrieval/contribution evidence available
  through the memory tools and the prompt's recent context. Prior versions of
  this prompt invoked `mimir reflection most-retrieved`; the maintenance shell
...
- **Applied-proposals audit** — read the newest
  `state/reports/applied-audit-YYYY-MM-DD.md` generated by the dedicated monthly
  in-process job. It carries the `## Effects of prior proposals` block:
  predicted-vs-measured signals for proposals applied 1-4 weeks ago. Missing or
  empty output is fine; only surface the block when there are rows to show. This
  closes the §12.2 double-loop without invoking the denied general `mimir` CLI.
...
Use the best available contributed/retrieved evidence from the memory tools and
recent context; look for atoms whose contribution evidence is improving.
...
Use the best available retrieval evidence from the memory tools and recent
context to identify stale candidates.
...
**No-trend fallback.** When no trend evidence is available for an atom (no
access history in the prior 90d, or fresh atoms before consolidation has labeled
...
2. Read the live file and write the archive to
   `memory/learnings-pending/YYYY-WNN.md` with `write_file`, preserving the
   content exactly. Then replace the live file with just the standard header.
