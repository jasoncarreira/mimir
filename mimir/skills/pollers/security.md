...
- **Runtime**: when a `pass_env` entry matches a deny-list pattern (`*_TOKEN`, `MIMIR_*`, etc.), the framework emits a `poller_env_passthrough_named_secret` event the first time it's pulled through. This audit is attempted at most once per poller name and env key per runner process lifetime, including across manifest reloads; restarting the runner resets the guard. Repeated fires and changed credential values do not re-emit this audit. Forwarding and diagnostic redaction still run on every fire. The guard retains only poller names and env key names, never values, and grows with distinct audited pairs rather than fire count. Grep `events.jsonl` for `poller_env_passthrough_named_secret` to audit runtime configuration, not occurrence counts. Read `poller` and `env_name` to identify which poller pulled which environment variable (for example, `env_name: "GITHUB_TOKEN"`). Credential values are never included. The legacy `key` field remains present but is redacted to `[REDACTED]` by the durable logger; use `env_name` for the variable name.
