...
describe("OpsRoute usage dashboard (#573)", () => {
  it("renders usage as the first-class resource tab and omits Chainlink from ops", async () => {
    renderRoute("/ops?tab=usage");

    expect(await screen.findByRole("heading", { name: "Usage Dashboard" })).toBeTruthy();
    expect((await screen.findByRole("tab", { name: "Usage" })).getAttribute("aria-selected")).toBe("true");
    ...
    expect(screen.getByLabelText("Daily token volume by token type")).toBeTruthy();
    expect(screen.getByText(/Codex Plus seven_day: 42\.0%/)).toBeTruthy();
  });
});
