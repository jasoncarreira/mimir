...
WEB_UI_CONFIG_FILENAME = "web_ui.json"
...
def read_web_ui_config(home: Path | None) -> dict[str, str]:
    """Agent-owned UI config: ``<home>/web_ui.json``.

    The agent edits this file itself (e.g. during onboarding) to set its display
    name and active skin. Read per-request like the other ``<home>/`` sources;
    a missing file or malformed JSON falls back to defaults. The skin value is
    passed through verbatim — the frontend validates it against its registry.
    """
    agent_name = DEFAULT_AGENT_NAME
    skin = DEFAULT_WEB_SKIN
    if home is not None:
        try:
            raw = json.loads((home / WEB_UI_CONFIG_FILENAME).read_text(encoding="utf-8"))
...
def ensure_web_ui_config(home: Path | None) -> None:
    """Create ``<home>/web_ui.json`` with defaults if it doesn't exist yet.
    ...
    if home is None:
        return
    path = home / WEB_UI_CONFIG_FILENAME
    if path.exists():
        return
    try:
