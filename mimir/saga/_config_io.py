...
era (``saga.toml`` / ``SAGA_CONFIG`` / ``SAGA_DATA_DIR``) — operators
already configure those, and the rename pass will introduce
``MIMIR_CONFIG`` aliases without breaking backward compat.

Config search order:
  1. $SAGA_CONFIG (explicit path to saga.toml)
  2. $SAGA_DATA_DIR/saga.toml (explicit data directory)
  3. ~/.saga/saga.toml (user-level config)
  4. <package_dir>/saga.toml (legacy, in-place development)

Data directory (for DB, caches) uses:
  1. $SAGA_DATA_DIR (explicit)
  2. ~/.saga/ (default)
...
# These mirror saga.toml exactly. Update both if you change a dial.
...
_DEFAULTS = {
    ...
    "storage": {
        "db_path": "saga.db",
        "metrics_db_path": "saga_metrics.db",
    ...
    "consolidation": {
        # 0.80 — reverted from the brief 0.75 stop (P34, 2026-05-02).
        # The simhi sweep (P40 + P47 reruns at 0.80, 2026-05-03) showed
        # that 0.75 over-merged across sessions: multi-session recall
        # dropped 4-5pp at 0.75 vs 0.80, especially on counting /
        # aggregation questions where retrieval at 0.75 missed answer
        # atoms that the looser clustering had collapsed. Temporal
        # reasoning also gained 3pp at 0.80. Net: tighter cutoff is
        # the better default; the paraphrase-merge gap P34 was chasing
        # is real but smaller than the cross-session penalty it
        # introduced.
...
def get_data_dir() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.saga/ (default user-level directory)
    """
    env = os.environ.get("SAGA_DATA_DIR")
    if env:
        data_dir = Path(env)
    else:
...
def _find_toml() -> Optional[Path]:
    """Locate saga.toml using the search order.

    1. $SAGA_CONFIG (explicit path)
    2. $SAGA_DATA_DIR/saga.toml
    3. ~/.saga/saga.toml
    4. <package_dir>/saga.toml (legacy / development)
    """
    ...
    # 2. Explicit data dir
    env_data = os.environ.get("SAGA_DATA_DIR")
    if env_data:
        p = Path(env_data) / "saga.toml"
        if p.exists():
            return p
    ...
    p = Path(__file__).parent / "saga.toml"
    if p.exists():
        return p
...
def _load_config() -> dict:
    ...
    if toml_path is None:
        ...
        logging.getLogger("saga.config").info(
            "No saga.toml found. Searched: $SAGA_CONFIG, $SAGA_DATA_DIR, "
            "~/.saga/saga.toml, <package>/saga.toml. Using defaults. "
            "Copy saga.example.toml to ~/.saga/saga.toml to configure."
        )
    ...
    if toml_path is not None and tomllib is not None:
        ...
        import logging
        logging.getLogger("saga.config").warning(
            "saga.toml found but tomllib unavailable (Python < 3.11 and no tomli installed). "
            "Using defaults."
        )
...
def _warn_unknown_keys(user_data: dict) -> None:
    ...
    Suppress with environment variable `SAGA_QUIET_CONFIG=1`.
    """
    if os.environ.get("SAGA_QUIET_CONFIG"):
        return
    import logging
    log = logging.getLogger("saga.config")
