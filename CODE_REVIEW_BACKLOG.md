...
- **Why it matters:** Slow RSS growth on long-lived mimir processes when consolidation cron fires across thread pool churn. The daemon-thread design means leaks aren't visible in `ps`, just memory. For a Pi-class deployment running mimir 24/7 this is real.
- **Approach (design decision):** Replace `threading.local` with a single shared `_PersistentClaudeCode` plus the existing `_submit_lock` (already there!) for serialization — saga calls are already CPU-bound and serialized through saga's SQLite, so per-thread parallelism wasn't gaining us much. **Recommended choice: single shared instance** since saga's mode is "many small calls in sequence," not "many parallel calls." ~40 LOC.
- **Effort:** S
- **Severity:** sev:med
