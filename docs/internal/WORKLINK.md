...
Epics are built by `feature-factory@0.10.4` through the lockstep
`opencode-feature-factory@0.10.4` adapter. OpenCode's `/feature` workflow owns
...
- **Controls**: Every control is `node <absolute feature-factory/bin/factory.js>`.
  Worklink admits the launcher only after package/adapter 0.10.4 verification and
...
| `feature_factory` | `opencode run ... --command feature " --autonomous --max-retries 5 <issue>"` | Epic adapter with Worklink-supervised OpenCode and absolute 0.10.4 controls. |
...
defaults:
  ...
  reaper_ttl_s: 86400       # >= 2 * timeout_s; backend + gates heartbeat; lower values warn + clamp
