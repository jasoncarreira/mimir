...
def temp_session(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("saga.session_dedup.SESSION_DIR", str(tmp_path))
    yield tmp_path


class TestRecordAndGetServed:
    def test_round_trip(self):
        from saga.session_dedup import record_served, get_served_ids
...
class TestClearSession:
    def test_clear(self):
        from saga.session_dedup import record_served, get_served_ids, clear_session
...
class TestSessionExpiry:
    def test_expires_old_data(self, tmp_path):
        from saga.session_dedup import get_served_ids, _session_file
