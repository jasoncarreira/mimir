...
def test_hands_contract_is_stdlib_only_and_in_client_closure() -> None:
    paths = module.module_paths(ROOT / "mimir")
    reached, analyses = module.closure(paths, module.ROOTS["client"])

    assert "mimir.acp.hands_contract" in reached
    analysis = analyses["mimir.acp.hands_contract"]
    assert module._first_party_imports(analysis, set(paths)) == set()
...
@pytest.mark.parametrize(("member", "source"), [
    ("mimir.acp.proxy", "import requests\nfrom . import profiles, credentials, transport, hosted, diagnostics, execution_scope\n"),
