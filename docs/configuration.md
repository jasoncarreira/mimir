...
| `MIMIR_FACTORY_ENTRYPOINT` | absolute path | `/opt/mimir-opencode/lib/node_modules/feature-factory/bin/factory.js` | Package-bound feature-factory 0.9.1 launcher. Worklink rejects relative, missing, unpinned, or adapter-version-mismatched entrypoints. |
...
| `defaults.epic_branch_prefix` | str | `epic/` | Compatibility-only field retained after integrated epic execution was removed; no 0.9.1 runtime consumes it. | `epic_branch_prefix: "epic/"` |
| `defaults.max_review_retries` | positive int | `3` | Compatibility-only parsed field; no 0.9.1 runtime consumes it. | `max_review_retries: 3` |
| `defaults.max_claim_attempts` | positive int | `3` | Compatibility-only parsed field; no 0.9.1 runtime consumes it. | `max_claim_attempts: 5` |
| `defaults.reviewer_backend` | backend name | value of `defaults.backend` | Compatibility-only parsed field from integrated epic review; no 0.9.1 runtime consumes it. | `reviewer_backend: opencode` |
...
0.9.1. It belonged to the removed distributed trusted-test runner. A value in
...
0.9.1. An unknown referenced backend fails configuration loading; stale settings
...
The retired `backends.feature_factory.bin`, `args`, `ready`, and `reviewer` keys are rejected with migration guidance. The image installs `feature-factory@0.9.1` and `opencode-feature-factory@0.9.1` under one npm prefix and registers only the OpenCode adapter. Runtime controls are `status`, `resume`, `heartbeat`, and run-ID-first `lock` actions; cancellation uses `mimir worklink stop` and never invokes a factory cancel transition.
