...
Usage:
    python -m saga.examples.synthetic_dataset
"""
...
def _mock_embeddings(monkeypatch_setattr):
    ...
    monkeypatch_setattr("saga.core.embed_text", _fake_embed)
    monkeypatch_setattr("saga.core.embed_query", _fake_embed)
    monkeypatch_setattr("saga.core.cached_embed_query", _fake_embed)
    monkeypatch_setattr("saga.core._cached_embed_query_import", lambda t: tuple(_fake_embed(t)))
    monkeypatch_setattr("saga.embeddings.embed_text", _fake_embed)
    return _fake_embed
...
async def main():
    ...
    import saga.config
    saga.config._config = None
    saga.config._config_loaded = False

    from pathlib import Path
    import saga.core
    _setattr("saga.core.DB_PATH", Path(db_path))

    # Also patch triples DB_PATH
    import saga.triples
    _setattr("saga.triples.DB_PATH", Path(db_path))
    ...
    from saga.core import store_atom, retrieve, hybrid_retrieve, get_stats
    from saga.core import get_db, run_migrations, metamemory_query
    ...
    from saga.triples import store_triple, store_triples_batch, retrieve_by_entity
    from saga.triples import get_triple_stats, format_triples_for_context, init_triples_schema
    ...
    from saga.decay import compute_all_retrievability, transition_states, budget_check
