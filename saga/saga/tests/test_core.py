...
class TestRetrieveMemoryTypeFilter:
    ...
    def test_raw_filter_returns_raw_atoms_under_faiss(
        self, monkeypatch,
    ):
        """retrieve(memory_type='raw') returns raw atoms and not
        observations. Session boundaries no longer live in atoms (migration 11 —
        store_session_boundary writes to sessions table), so no source_type
        exclusion is needed in the FAISS post-filter."""
        from saga.core import store_atom, retrieve
        self._setup_with_distinct_embeddings(monkeypatch)

        # One ordinary raw atom on the topic.
