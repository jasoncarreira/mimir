...
class TestHealth:
    ...
    def test_health_no_auth_required(self, client, monkeypatch):
        ...
        monkeypatch.setenv("SAGA_API_KEY", "test-secret")
        rv = client.get("/v1/health")
        assert rv.status_code == 200
        data = rv.json()
        assert data["status"] == "ok"
...
class TestQuery:
    ...
    def test_query_two_tier_request_field(self, client, monkeypatch):
        ...
        monkeypatch.setattr(saga.core, "hybrid_retrieve", fake_hybrid_retrieve)
...
class TestStats:
    def test_stats_endpoint(self, client, monkeypatch):
        ...
        import saga.core
        monkeypatch.setattr(saga.core, "get_stats", lambda: {
            "total_atoms": 42,
            "active": 30,
            "fading": 10,
            "dormant": 2,
...
class TestFeedback:
    ...
    def test_feedback_success(self, client, monkeypatch):
        import saga.core
        monkeypatch.setattr(saga.core, "mark_contributions", lambda ids, text, sid=None: {
...
class TestApiKey:
    ...
    def test_api_key_success(self, client, monkeypatch):
        ...
        import saga.core
        monkeypatch.setattr(saga.core, "get_stats", lambda: {"total_atoms": 0})

        rv = client.get("/v1/stats", headers={"X-API-Key": "test-secret"})
        assert rv.status_code == 200

    def test_no_api_key_configured(self, client, monkeypatch):
        """When no SAGA_API_KEY is set, all requests should be allowed."""
        monkeypatch.delenv("SAGA_API_KEY", raising=False)
...
class TestDecay:
    def test_decay_endpoint(self, client, monkeypatch):
        import saga.decay
        monkeypatch.setattr(saga.decay, "run_decay_cycle", lambda: {
...
class TestTriples:
    ...
    def test_graph_traversal(self, client, monkeypatch):
        import saga.triples
        monkeypatch.setattr(saga.triples, "graph_traverse", lambda e, max_hops=3: {
...
class TestContradictions:
    def test_contradictions_triples_mode(self, client, monkeypatch):
        import saga.triples
        monkeypatch.setattr(saga.triples, "detect_contradictions", lambda: [
...
class TestReplay:
    def test_replay_endpoint(self, client, monkeypatch):
        import saga.core
        monkeypatch.setattr(saga.core, "episodic_replay", lambda **kwargs: {
...
class TestRecentSessions:
    def test_returns_sessions(self, client, monkeypatch):
        import saga.core
        monkeypatch.setattr(saga.core, "get_last_sessions", lambda **kw: [
    ...
    def test_passes_filters(self, client, monkeypatch):
        ...
        def fake(**kw):
            captured.update(kw)
            return []
        monkeypatch.setattr(saga.core, "get_last_sessions", fake)
...
class TestMostRetrieved:
    def test_returns_atoms(self, client, monkeypatch):
        import saga.core
        monkeypatch.setattr(saga.core, "get_most_retrieved", lambda **kw: [
