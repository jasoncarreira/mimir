...
* Response shape (per response, milliseconds):
        ...
        "model_remains": [
          {
            ...
            "current_interval_usage_count": 4152,
...
* **``usage_count`` is REMAINING, not USED.** Confirmed by the
  reference js poller in ``~/projects/odin/usage``. We compute
  ``used = max(0, total - remaining)`` and surface ``utilization =
  used / total``.
...
DEFAULT_MODEL_NAME = "MiniMax-M*"
"""The text-completion plan bucket. Minimax groups gpt-style models
under this glob (matches MiniMax-M2, MiniMax-M2.5, MiniMax-M2.7,
MiniMax-M2.7-highspeed, etc.). Image / speech / music plans have
separate entries with their own quotas."""
...
async def poll_once(
    ...
    *,
    ...
    if not interval_total and not weekly_total:
        # Both totals zero = API key's plan doesn't cover this model.
        # Writing utilization=0 here would be a lie — surface it.
        await log_event(
            "minimax_usage_failed",
            stage="empty_plan",
            error=(
                f"both interval and weekly totals are 0 for "
